...
async function getdata() {
  ...
  for (let pultosPSW of state.pultosokPSW) {
    ...
    if (index == 4 && state.pultosokPSW.length < 7) {
      pultosokHTML += `<div class="card bg-danger text-white ml-2 mt-4 p-3" style="width:700px" id=${index}>
            ...
            <br>
            <label>A TERV :</label>
            <input type="number" value= "${pultosPSW.pin}" id=newPin${
        index + 1
      }>
            </div>`;
    }
    ...
    if (index == 5) {
      ...
      pultosokHTML += `<div class="text-center" style="width:700px">
            <br>
            <label>A TERV :</label>
    ...
    if (index == 6) {
      let BD = parseInt(pultosPSW.pin);
      /* let BD = (pultosPSW.pin).toLocaleString("hu-HU", {
                maximumFractionDigits: 0,
            }) */
      pultosokHTML += `<div class="text-center" style="width:700px">
            
            <label>Betétdíj :</label>
            <input type="text" value= "${BD}" id=newPin${index}>
            </div>`;
    }
    index++;
  }
...
function pultosokAdminTarol() {
  ...
  BD = parseInt(document.getElementById("newPin6").value);
  data.push({ pin: BD, name: "BD" });
  updatePultosokFe();
