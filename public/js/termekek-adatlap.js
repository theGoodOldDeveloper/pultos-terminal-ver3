...
function rendertermekek() {
  ...
  for (let termek of state.termekek) {
    productPercent = Math.round(
      ((termek.elar - termekHaszon[termek.id]) / termekHaszon[termek.id]) * 100
    );
    termekekHTML += `<tr >
        <td>${termek.id}</td>
        ...
        <td class="text-right">${Math.round(
          termek.elar - termekHaszon[termek.id]
        )}</td>
        ...
        <td><checkbox type="checkbox" name="visszavalt" id="tVisszavalt" />${
          termek.emailsend == 1 ? "IGEN" : ""
        }</td>
  ...
  $("#addNewBtn")
    ...
    .on("click", function () {
        ...
        "newTermek"
            ...
            <thead>
                ...
                <tr>
                    ...
                    <td>
                    <input type="checkbox" name="visszavalt" id="tVisszavalt" />
                  </td>
                    <td>
      ...
      $("#cosnsoleWrite").click(function () {
        ...
        var visszavalt = document.getElementById("tVisszavalt").checked ? 1 : 0;
        /* 
        const checkbox = document.getElementById('tVisszavalt');
const value = checkbox.checked ? 1 : 0;
        */
        ...
        inserttermekek(nev, elar, btncolor, visszavalt);
      });
    });
  $(".updateBtn").click(function () {
    ...
    origVisszavalt = state.termekek[arrowIndex].emailsend;
    ...
    document.getElementById("newVisszavalt").checked =
      state.termekek[arrowIndex].emailsend === 1; //BUG
...
function renderOsszetevok() {
  ...
  $("#addOsszetevo").click(function () {
    ...
    document.getElementById("selectOsszetevo").innerHTML = selectOsszetevokHTML;
    ...
    $(".selected").click(function (e) {
      ...
      async function insertOsszetevokMySQL() {
        ...
        felhasznaltmennyiseg = document.getElementById("felhasznalt").value;
...
function inserttermekek(vnev, velar, vbtncolor, vvisszavalt) {
  const nev = vnev;
  const elar = velar;
  const visszavalt = vvisszavalt;
...
function updatetermekek() {
  ...
  const emailsend = document.getElementById("newVisszavalt").checked ? 1 : 0;
  ...
  async function updateMySQL() {
    ...
    state.termekek[arrowIndex].elar = elar;
    state.termekek[arrowIndex].btncolor = btncolor;
    state.termekek[arrowIndex].emailsend = emailsend;
    rendertermekek();
  }
}
  ...
  for (var termek of state.termekek) {
    for (var osszetevo of state.osszetevok) {
      if (osszetevo.termek_id == termek.id) {
        termekHaszon[termek.id] +=
          osszetevo.felhasznaltmennyiseg *
          alapanyagBeszar[osszetevo.alapanyag_id];
      }
    }
  }
}
