...
function renderAlapanyagokData() {
  ...
  for (alapanyag of state.alapanyagok) {
    if (alapanyag.nev.toLowerCase().search(keresValue.toLowerCase()) >= 0) {
      ...
      //VERSION-2:
      nowKeszletDB = Math.trunc(
        Math.round((alapanyag.keszletsum / alapanyag.kiszereles) * 100) / 100
      );
      nowKeszletPULT =
        Math.round(
          alapanyag.keszletsum - nowKeszletDB * alapanyag.kiszereles * 100
        ) / 100;
      //VERSION-3:
      /* <td>${index}</td> */
      alapanyag.emailsend
        ? (alapanyagemailsend = "igen")
        : (alapanyagemailsend = "");
      alapanyagokHTML += `<tr >
        <td>${alapanyag.id}</td>
        <td>${alapanyag.nev}</td>
  ...
  $(".updateBtn").click(function (e) {
    ...
    nowKeszletDB = Math.trunc(
      Math.round(
        (state.alapanyagok[clickIndex].keszletsum /
    ...
    nowKeszletPULT =
      state.alapanyagok[clickIndex].keszletsum -
      Math.round(
        nowKeszletDB * state.alapanyagok[clickIndex].kiszereles * 100
      ) /
        100;
    nowKeszletPULT = Math.round(nowKeszletPULT * 100) / 100;
    //VERSION-2:

    document.getElementById("nev").value = state.alapanyagok[clickIndex].nev;
    ...
    document.getElementById("keszlet").value = nowKeszletDB;
    ...
    document.getElementById("pultbanvan").value = state.pultbanVan[id];
    document.getElementById("pult").value = nowKeszletPULT;
    document.getElementById("beszar").value =
      state.alapanyagok[clickIndex].beszar;
    document.getElementById("emailsend").value =
      state.alapanyagok[clickIndex].emailsend;
